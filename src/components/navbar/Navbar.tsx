...
import { NavbarProps, NavAction } from "@/types/models/navbar"; // Adjusted import path
    ...
    switch (action.type) {
      ...
      case 'button':
        return (
          <button
            key={index}
            onClick={action.onClick? action.onClick : () => { if (action.href) window.location.href = action.href; }}
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded shadow-lg z-10">
                {action.items.map((item, itemIndex) => (
                  <Link
                    key={itemIndex}
                    href={item.href}
                    className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center text-text-light dark:text-text-dark"
                    onClick={() => {
